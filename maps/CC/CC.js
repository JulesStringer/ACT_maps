...
function showfeaturedetail(feature) {
    ...
    var body = '';
    if (properties.layerName == 'CCAreas') {
        $('#areacode').text(properties['code']);
        $('#areaname').text(properties['name']);
        var carbon_cutters = properties['carbon_cutters'];
        var str = '';
        if (!carbon_cutters) {
            str += 'No Carbon Cutters';
        } else if (carbon_cutters == 1) {
            str += '1 Carbon Cutter';
        } else {
            str += carbon_cutters + ' Carbon Cutters';
        }
        $('#carbon_cutters').text(str);
        var email = properties['email'];
        if (email) {
            $('#email').text('Contact ' + properties['name'] + ' carbon cutters');
...
function onclick(mapdisp, pt) {
    ...
    var features = mapdisp.getFeaturesIntersecting("CCAreas", coords);
